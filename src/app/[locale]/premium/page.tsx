...
export default function PremiumPage() {
  ...
  useEffect(() => {
    if (expiresAt) {
      ...
      setIsExpiringSoon(date.getTime() - Date.now() < 7 * 24 * 60 * 60 * 1000);
    } else {
        ...
        <div className="mx-auto w-full max-w-lg space-y-6">
          <Card>
            <CardContent className="pt-6 text-center">
              ...
              <p className="mt-2 text-sm text-muted-foreground">{t("clearedDesc")}</p>
              <Link
                href="/unlock"
          ...
          <Card className="border-dashed">
            <CardContent className="pt-6 text-center space-y-4">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">{t("notPremiumDesc")}</p>
              </div>
              <UnlockButton className="w-full text-center" />
              <p className="text-xs text-muted-foreground">
                {t("paidPrompt")}
                <Link href="/unlock" className="text-primary hover:underline">
                  {t("activateOrder")}
                </Link>
              </p>
            </CardContent>
  ...
  return (
    <div className="flex min-h-dvh flex-col px-4 py-8">
      <div className="mx-auto w-full max-w-lg space-y-6">
        ...
        {/* Status Card */}
        <Card
          className={`${expiresDate && isExpiringSoon ? "border-yellow-200 bg-yellow-50 dark:border-yellow-800 dark:bg-yellow-950" : "border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-950"}`}
        >
          <CardContent>
            <div className="flex items-center gap-3">
              <div
                className={`flex h-10 w-10 items-center justify-center rounded-full ${expiresDate && isExpiringSoon ? "bg-yellow-600" : "bg-green-600"}`}
              >
                <Check className="h-5 w-5 text-white" />
              </div>
              <div>
                <p
                  className={`font-semibold ${expiresDate && isExpiringSoon ? "text-yellow-800 dark:text-yellow-200" : "text-green-800 dark:text-green-200"}`}
                >
                  {t("statusActivated")}
                </p>
                {expiresDate ? (
                  <p
                    className={`text-sm ${isExpiringSoon ? "text-yellow-600 dark:text-yellow-400" : "text-green-600 dark:text-green-400"}`}
                  >
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center gap-1 text-xs text-muted-foreground">
              ...
              {deviceCount >= maxDevices && (
                <span className="text-yellow-600 dark:text-yellow-400">{t("deviceFull")}</span>
              )}
            </div>
        ...
        {licenseKey && (
          <Card className="border-red-200 dark:border-red-800">
            ...
            <CardContent>
              <p className="text-xs text-muted-foreground mb-3">{t("dangerDesc")}</p>
