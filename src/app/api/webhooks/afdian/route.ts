...
export async function POST(request: Request) {
  try {
    ...
    if (order.status !== 2) {
      console.log(
        `[afdian-webhook] Order ${order.out_trade_no} status=${order.status}, skipping`,
      );
    ...
    if (!tier.shouldUnlockPremium) {
      ...
      console.log(
        `[afdian-webhook] Order ${order.out_trade_no} is support tier (${order.total_amount})`,
      );
    ...
    if (result.success) {
      ...
      console.log(
        `[afdian-webhook] ${isRenewal ? "Existing" : "New"} license ${result.licenseKey} for order ${order.out_trade_no} (${tier.label})`,
      );
      ...
      try {
        ...
        console.error(
          `[afdian-webhook] Failed to send DM for order ${order.out_trade_no}`,
        );
      }
    } else {
      console.error(
        `[afdian-webhook] Failed to create license for order ${order.out_trade_no}: ${result.reason}`,
      );
    }
