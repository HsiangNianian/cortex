...
2. **分享回流**
   ...
   - 着陆页增加 CTA 差异（新用户 → 开始测试；回流用户 → 再测一次）

3. **轻量提醒**
   - 结果页引导："7 天后复测，追踪你的变化"
   - 利用浏览器 Notification API（需用户授权）
...
Phase 0 已完成，Phase 0.5 数据基建已完成。目前在 Phase 0.75（留存与增长）。
Phase 1 之前的"留存"环节不可或缺——在投入自适应算法前，需要确认用户有复测习惯，
否则 Phase 1 的画像系统没有数据可学习。
