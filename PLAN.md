...
Phase 0 → 0.9 全部完成。Phase 1 Week 1-3 全部完成。
...
- LLM 题目生成管线（DeepSeek + OpenAI SDK + Redis 缓存 + 限频）
...
### Phase 1 — Week 4：打磨 + 向后兼容 🎯

- [ ] 统计页展示 `estimationMethod` 差异信息
- [ ] 历史趋势图 Phase 0/Phase 1 结果同图兼容显示
- [ ] OG 分享、ResultPhase 对接 `estimationMethod`
- [ ] `/api/results` 和 `/api/stats` 添加可选 IRT 字段
- [ ] 自适应测试流程接入 useTestState（走 ADAPTIVE_MODE flag）
