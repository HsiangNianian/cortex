...
function extractExplanations(
  source: string,
): Array<{ start: number; end: number; text: string; options: string[] }> {
...
function fixExplanation(text: string, options: string[]): string {
  ...
  result = result.replace(
    /(?:仅|只)?有?\s*(?:选项)?\s*([A-Da-d])\s*(正确|符合|是正确答案)/g,
    (_, letter: string, suffix: string) => {
      const idx = letterToIndex(letter);
      if (idx >= 0 && idx < options.length) {
  ...
  result = result.replace(
    /([，,])\s*([A-Da-d])\s*(正确|是正确答案|为正确答案)\b/g,
    (_, sep: string, letter: string, suffix: string) => {
      const idx = letterToIndex(letter);
      if (idx >= 0 && idx < options.length) {
  ...
  result = result.replace(
    /([A-Da-d])\s*(项|是)\s*(正确|为正确答案)\b/g,
    (_, letter: string, mid: string, suffix: string) => {
      const idx = letterToIndex(letter);
      if (idx >= 0 && idx < options.length) {
  ...
  result = result.replace(
    /([，,。\s])([A-Da-d])\s*(符合|是|为)\b/g,
    (_, prefix: string, letter: string, suffix: string) => {
      const idx = letterToIndex(letter);
      if (idx >= 0 && idx < options.length) {
  ...
  result = result.replace(
    /([，,。\s])([A-Da-d])\s*(是|为)\s*(最|更)/g,
    (_, prefix: string, letter: string, mid: string, adv: string) => {
      const idx = letterToIndex(letter);
      if (idx >= 0 && idx < options.length) {
  ...
  result = result.replace(
    /[，,。\s]([A-Da-d])\s*(正确|错误|与|完全|是|为)\b/g,
    (match: string, letter: string, suffix: string) => {
      const idx = letterToIndex(letter);
      if (idx >= 0 && idx < options.length) {
